...
def ogr2ogr(url: str, file_path: Path, records: int | None) -> CompletedProcess[bytes]:
    ...
    dst_dataset = file_path.with_suffix(".gpkg")
...
def is_correct_geom_type(file_path: Path, geom_type: int) -> bool:
    ...
    result = run(
        ["ogrinfo", file_path.with_suffix(".gpkg")],
        capture_output=True,
        check=False,
    )
...
def to_parquet(file_path: Path) -> None:
    ...
    Args:
        file_path: Name of the downloaded layer.
    """
    src_dataset = file_path.with_suffix(".gpkg")
    dst_dataset = file_path.with_suffix(".parquet")
    dst_dataset.unlink(missing_ok=True)
    gdf = read_file(src_dataset).convert_dtypes()
    for column in gdf.select_dtypes(include="int64").columns:
        if gdf[column].isna().all():
            gdf[column] = gdf[column].astype("string")
        else:
            gdf[column] = gdf[column].astype("int32")
    for column in ["adm_level", "version_no"]:
        if column in gdf.columns:
            gdf[column] = gdf[column].astype("int32")
    for column in ["valid_on", "valid_to"]:
        if column in gdf.columns:
            gdf[column] = gdf[column].astype("datetime64[ns]").astype("date32[pyarrow]")
    for column in ["OBJECTID", "Shape__Area", "Shape__Length"]:
        if column in gdf.columns:
